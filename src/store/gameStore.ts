...
export const useGameStore = create<GameState>()(
  persist(
    (set, get) => ({
      ...
      startGame: () => {
        const state = get();
        const filteredWords = defaultWords.filter(
          (w) => w.difficulty === state.difficulty
        );
      ...
      endGame: () => {
        ...
        const timePlayed =
          (new Date().getTime() - state.timeStarted.getTime()) / 1000;
        const averageTime =
          state.wordList.length > 0 ? timePlayed / state.wordList.length : 0;
